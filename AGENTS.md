...
│   │   ├── PdfViewer.tsx              # PDF 渲染协调层、选区、键盘导航
...
│   │   ├── TitleBar.tsx               # 自定义标题栏（data-tauri-drag-region + RecentFilesBar + 快捷键/设置入口 + 窗口控制 + quickToggles 中部插槽）
...
│   │   ├── useFileDrop.ts             # 系统文件拖放（onDragDropEvent，拖 PDF 进窗口即打开；遮罩带事件流停顿/blur 看门狗）
...
│   ├── pdf-space-pan.spec.ts          # Space 按住拖拽 pan（连续滚动模式 + 不按 Space 时选择不受影响）
...
- `read_pdf_bytes(filePath: string)`：读取 PDF 原始字节；同一遍读取中算好 SHA-256 并写入 `pdf_hash_cache` 预热（`warm_pdf_hash_cache`），随后的 `get_pdf_hash` 仅 metadata 校验即命中缓存，局域网文件整条打开链路只需一遍网络传输。
...
├── useFileDrop：isFileDragOver                    # 系统文件拖放（拖 PDF 进窗口即打开 + 全窗口遮罩，含看门狗兜底隐藏）
...
- Playwright 启动 `npm run dev` 作为 webServer，访问 `http://localhost:1420`，共 8 个 spec：
  ...
  - `pdf-rapid-zoom.spec.ts`：快速缩放回归。
  - `pdf-selection-translate.spec.ts`：选区翻译流程。
  - `pdf-space-pan.spec.ts`：Space 按住拖拽 pan（连续滚动模式下滚动位置不弹回、不按 Space 时拖拽仍是文字选择）。
