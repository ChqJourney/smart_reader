...
> 状态：本文为完整目标架构，正文设计保留作参考。当前（v1.0.0）Agent Tools 已落地：`list_open_pdfs` / `read_pdf_page` / `search_in_pdf` 三工具 + 视觉模型页面截图工具 `screenshot_pdf_page`，在解读 / 自定义解读 / 追问（含无选区自由提问）时启用（轮次上限默认 20），LLM 请求经 Rust 后端代理 `llm_proxy.rs` 发出；Clause 索引、术语表、测试清单、表格多模态读取等仍为后续规划。
...
用户打开 PDF（支持多 Tab，最多 10 个）
    │
    ▼
[本地 PDF 渲染层]  ← 连续滚动阅读、文本选区、缩放、页码跳转
    │
...
- `PdfViewer.tsx`：本地渲染、连续滚动模式、键盘导航、文本选区、annotation 渲染。
