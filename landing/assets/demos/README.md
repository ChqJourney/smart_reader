...
> 录制 `link-preview` 前：功能默认关闭，需先在设置「功能设置」页开启「条款链接悬停预览」；且 PDF 必须带文档内链接（交叉引用）。测试 PDF 可运行 `node scripts/gen-sample-link-pdf.mjs` 生成 `e2e/fixtures/sample-links.pdf`（含多条"见 X.Y"内部引用链接）。
...
| `link-preview`    | 功能 02   | 建议   | 悬停"见 5.2"类条款引用约 2s → 画中画小窗弹出并定位到引用处 → 拖动小窗 → 图钉固化 → 再悬停另一处引用弹出第二个窗口 |
